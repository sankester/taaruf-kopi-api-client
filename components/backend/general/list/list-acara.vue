<template>
  <v-card>
    <v-toolbar card dense color="transparent">
      <v-toolbar-title><h4>List Acara Terbaru</h4></v-toolbar-title>
      <v-spacer></v-spacer>
      <v-btn flat color="indigo">
        <v-icon class="mr-1">list</v-icon>Selengkapnya
      </v-btn>
    </v-toolbar>
    <v-divider></v-divider>
    <v-card-text class="pa-0">
      <template>
        <v-data-table :headers="headers" :items="list" hide-actions class="elevation-0">
          <!--looping data-->
          <template slot="items" slot-scope="props">
            ...
            <td class="text-xs-center">
              <v-btn flat icon color="info" nuxt :to="'acara/' + props.item.id">
            ...
            <v-layout align-center justify-center fill-height row wrap class="py-2">
              <v-flex md12 lg12 xs12 sm12>
                <v-alert
                  ...
                  outline
                >
                  Data belum tersedia, pergi ke menu acara untuk menambah acara.
                </v-alert>
              </v-flex>
            </v-layout>
          </template>
