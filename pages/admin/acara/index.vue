    ...
    <v-flex xs12>
      ...
      <v-card style="min-height: 250px !important;">
        ...
        <v-toolbar dark color="primary">
          <v-toolbar-title class="white--text">List Acara</v-toolbar-title>
          <v-spacer></v-spacer>
          <!--button add-->
          <v-btn nuxt to="/admin/acara/new" outline round dark v-if="!isMobile">
            <v-icon>add</v-icon> Tambah Acara
          </v-btn>
        </v-toolbar>
        <!--floating add button-->
        <v-card-text style="position: relative" v-if="isMobile">
          <v-fab-transition>
            <v-btn nuxt to="/admin/acara/new" color="success" class="mb-5" dark fab fixed bottom right>
              <v-icon>add</v-icon>
              <v-icon>close</v-icon>
            </v-btn>
          </v-fab-transition>
        ...
        <v-container fluid>
          ...
          <!--show list user-->
          <AcaraList @delete="deleteAcara($event)" :headers="acaraHeaders" :list="acaraList" :loading="loadingButton" :pagination="pagination" @reload="initialize" @loadMore="loadMore"/>
        </v-container>
      </v-card>
    </v-flex>
  </v-layout>
...
<script>
  // impoer component
  import AcaraList from "@/components/backend/acara/AcaraList";
  ...
  export default {
    ...
    computed: {
      // get list user
      acaraList() {
        return this.$store.getters.getListAcara;
      },
      ...
      // get pagination data
      pagination() {
        return this.$store.getters.getAcaraPagination;
      }
    },
    mounted() {
    ...
    methods: {
      ...
      initialize() {
        ...
        this.$awn.asyncBlock(
          ...
          this.$store.dispatch("getListAcara").catch(error => {
      ...
      loadMore(link) {
        ...
        this.$store.dispatch("loadMoreAcara", link)
          .then(res => {
            ...
            // update list user
            this.$store.commit("updateListAcara", res.data);
            this.$store.commit("setLastAcaraPagination", res.meta.pagination);
          })
          .catch(error => {
            // disable loading button
            this.loadingButton = false;
      ...
      resetSearch() {
        ...
        this.$store.commit("setKeywordAcara", " ");
        this.keyword = "";
        // load data user
        this.initialize();
      },
      ...
      search() {
        ...
        this.$store.commit("setKeywordAcara", this.keyword);
        // get data
        this.initialize();
      },
      // delete user
      deleteAcara(acara) {
        // show confirm dialog
        this.$vuetifyConfirmDialog.open("Konfirmasi Perintah", "Apakan anda yakin menghapus acara ini ?", "Tidak", "Iya").then(confirm => {
          if (confirm) {
            //show waiting asyncronus loading indicator
            this.$awn.asyncBlock(
              // get data user from API Server
              this.$store.dispatch('deleteAcara', acara.id).then((res) => {
                // show notification
                this.$awn.success('<h4>' + res.message + '</h4>')
                // delete in store
                this.$store.commit('deleteAcara', acara)
              }).catch((error) => {
                // default error message
    ...
    head: {
      titleTemplate: "Manajemen Acara - %s",
      meta: [
        {charset: "utf-8"},
