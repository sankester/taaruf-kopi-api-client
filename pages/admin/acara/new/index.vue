  ...
  <v-layout row wrap>
    <v-flex xs12>
      ...
      <v-breadcrumbs divider="/">
        ...
        <v-spacer></v-spacer>
        <v-breadcrumbs-item class="xs12" exact-active-class="/admin/acara" exact replace v-for="breadcumb in breadcumbs" :key="breadcumb.text" :disabled="breadcumb.disabled" :to="breadcumb.link">
          {{ breadcumb.text }}
        </v-breadcrumbs-item>
      </v-breadcrumbs>
      <!--form input-->
      <v-card>
        ...
        <v-toolbar dark color="primary">
          <div class="headline">Form input acara</div>
          <v-spacer></v-spacer>
        ...
        <FormAcara ref="formAcara" @insert="insert($event)" @reset="reset"/>
      </v-card>
    </v-flex>
  </v-layout>
</template>
    ...
    methods: {
      ...
      insert(acara) {
        ...
        this.$awn.asyncBlock(
          ...
          this.$store.dispatch('saveAcara', acara).then((res) => {
            // show notification
            this.$awn.success('<h4>'+ res.message +'</h4>')
            // disable alert
            this.alert.status = false
            // reset form
            this.$refs.formAcara.reset();
          }).catch((error) => {
